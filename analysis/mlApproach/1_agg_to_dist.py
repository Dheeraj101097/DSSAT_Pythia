...
input_folder = 'val'        # Folder containing your CSV files
...
for filename in os.listdir(input_folder):
    if filename.endswith('.csv'):
        ...
        # Keep relevant columns change column names seieng csv file it gives error
        points_with_district = gdf_joined[['LAT', 'LONG', 'District','YEAR', 'PREDICTED']]
        
        # # Group by district and year, calculate mean yield
        # summary = points_with_district.groupby(['District', 'YEAR'], as_index=False)['PREDICTED'].sum()
        # summary = summary.rename(columns={'HWAM': 'TOTAL_HWAM'})

        # Group by district and calculate overall mean yield across all years
        district_mean_yield = points_with_district.groupby('District', as_index=False)['PREDICTED'].mean()
